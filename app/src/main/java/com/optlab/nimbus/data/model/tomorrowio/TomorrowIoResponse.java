...
public record TomorrowIoResponse(@SerializedName("data") Data data) {
     ...
     */
    public static List<ForecastResponse> mapToWeatherResponse(@NonNull TomorrowIoResponse response) {
        // List is used to store the weather summaries (daily weather data)
        List<ForecastResponse> weatherSummaries = new ArrayList<>();
        ...
        response.data() // Get the data from the response
                ...
                .forEach(
                        interval -> {
                            ...
                            ForecastResponse weather = new ForecastResponse();
